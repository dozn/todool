...
changelog_text_display_message :: proc(
	element: ^Element,
	msg: Message,
	di: int,
	dp: rawptr,
	...
	case .Paint_Recursive:
		{
			...
			if len(text) == 0 {
				...
				for line in strings.split_lines_iterator(&iter) {
					...
					render_string(target, x + tabs * tab_scaled, y, line[tabs:])
...
changelog_text_display_init :: proc(
	...
	res = element_init(
		Changelog_Text_Display,
		parent,
		flags,
		changelog_text_display_message,
		context.allocator,
	)
...
changelog_spawn :: proc(du: u32 = COMBO_EMPTY) {
...

	{
...

		{
			...
			changelog.checkbox_include_canceled = checkbox_init(
				p2,
				{.HF},
				"Include Canceled Tasks",
				true,
			)
