...
timing_timestamp_extract :: proc(text: []byte) -> (stamp: Timestamp, ok: bool) {
...
timing_timestamp_is_today :: proc(stamp: Timestamp) -> bool {
	year, month, day := time.date(time.now())
	return year == stamp.year && int(month) == stamp.month && day == stamp.day
}
	...
	case .Paint_Recursive:
		{
			...
			render_string_rect_store(
				target,
				element.bounds,
				strings.to_string(td.builder),
				&td.rendered_glyphs,
			)
		}
	...
	case .Layout:
		{
			...
			switch time_date_format {
			case .US:
				fmt.sbprintf(b, "%4d-%2d-%2d", year, int(month), day)
			case .EUROPE:
				fmt.sbprintf(b, "%2d.%2d.%4d", day, int(month), year)
			}
...
time_date_render_highlight_on_pressed :: proc(target: ^Render_Target, clip: RectI) {
...
menu_date_day_set :: proc(value: int) {
	...
	offset_year, offset_month, _ := menu_date_timing()
...
menu_date_buttons_set :: proc() {
	...
	first_in_month, _ := time.datetime_to_time(offset_year, int(offset_month), 1, 12, 0, 0)
...
menu_date_spawn :: proc(ptr: ^time.Time, x, y: int) {
	...
	// top bar
	{
		...
		b1.invoke = proc(button: ^Icon_Button, data: rawptr) {
			_, _, offset_day := menu_date_timing()
			menu_date_day_offset -= offset_day
			menu_date_buttons_set()
		}
	...
	week_names := [7]string{"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"}
