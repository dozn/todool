...
render_target_init :: proc(window: ^sdl.Window) -> (res: ^Render_Target) {
	...
	// shader loading
	shader_ok: bool
	res.shader_program, shader_ok = gl.load_shaders_source(
		string(shader_vert),
		string(shader_frag),
		false,
	)
	...
	res.attribute_roundness_and_thickness = u32(
		gl.GetAttribLocation(res.shader_program, "i_roundness_and_thickness"),
	)
	...
	res.textures[.Fonts].uniform_sampler = gl.GetUniformLocation(
		res.shader_program,
		"u_sampler_font",
	)
...
render_target_end :: proc(target: ^Render_Target, window: ^sdl.Window, width, height: int) {
	...
	gl.VertexAttribPointer(
		target.attribute_add,
		4,
		gl.FLOAT,
		true,
		size,
		offset_of(Render_Vertex, add),
	)
	gl.VertexAttribIPointer(
		target.attribute_roundness_and_thickness,
		1,
		gl.UNSIGNED_INT,
		size,
		offset_of(Render_Vertex, roundness),
	)
	gl.VertexAttribIPointer(
		target.attribute_kind,
		1,
		gl.UNSIGNED_INT,
		size,
		offset_of(Render_Vertex, kind),
	)
	...
	for group in &target.groups {
		...
		if vertice_count != 0 {
			...
			gl.BufferData(
				gl.ARRAY_BUFFER,
				vertice_count * size_of(Render_Vertex),
				root,
				gl.STREAM_DRAW,
			)
...
// no
render_rect :: proc(target: ^Render_Target, rect: RectI, color: Color, roundness: int = 0) {
...
// NOTE ASSUMING RGBA
texture_update_handle :: proc(handle: u32, data: ^byte, width: int, height: int) {
...
texture_bind :: proc(target: ^Render_Target, kind: Texture_Kind) {
	texture := &target.textures[kind]
	gl.Uniform1i(texture.uniform_sampler, i32(kind))
	gl.ActiveTexture(gl.TEXTURE0 + u32(kind))
	gl.BindTexture(gl.TEXTURE_2D, texture.handle)
}
...
render_string :: proc(target: ^Render_Target, x, y: int, text: string) -> f32 {
	group := &target.groups[len(target.groups) - 1]
	state := fontstash.__getState(&gs.fc)
...
render_icon :: proc(target: ^Render_Target, x, y: f32, icon: Icon) -> f32 {
	ctx := &gs.fc
...
render_icon_rect :: proc(target: ^Render_Target, rect: RectI, icon: Icon) -> f32 {
	ctx := &gs.fc
	state := fontstash.__getState(&gs.fc)
	group := &target.groups[len(target.groups) - 1]
	font := fontstash.__getFont(ctx, state.font)
...
render_texture_from_kind :: proc(
	...
	vertices[0] = {
		pos_xy = {f32(r.l), f32(r.t)},
		uv_xy  = {0, 0},
	}
	vertices[1] = {
		pos_xy = {f32(r.r), f32(r.t)},
		uv_xy  = {1, 0},
	}
	vertices[2] = {
		pos_xy = {f32(r.l), f32(r.b)},
		uv_xy  = {0, 1},
	}
	vertices[5] = {
		pos_xy = {f32(r.r), f32(r.b)},
		uv_xy  = {1, 1},
	}
...
render_sine :: proc(target: ^Render_Target, r: RectI, color: Color) {
	group := &target.groups[len(target.groups) - 1]
	vertices := render_target_push_vertices(target, group, 6)
...
// render a string at arbitrary xy
render_string_store :: proc(target: ^Render_Target, x, y: int, text: string) -> f32 {
	group := &target.groups[len(target.groups) - 1]
	state := fontstash.__getState(&gs.fc)
	iter := fontstash.TextIterInit(&gs.fc, f32(x), f32(y), text)
	q: fontstash.Quad
